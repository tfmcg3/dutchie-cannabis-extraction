#!/usr/bin/env python3
"""
Optimized CLI Script for Dutchie Dispensary Extraction

Enhanced command line interface for extracting cannabis dispensary data
from the Dutchie platform with proper infinite scroll handling.

Usage:
    python scripts/extract_dispensary_optimized.py quincy-cannabis-quincy-retail-rec
    python scripts/extract_dispensary_optimized.py dispensary-slug --categories flower edibles --output results/
"""
...
from extractors.dutchie_extractor_optimized import DutchieExtractorOptimized
from analysis.competitive_intelligence import CompetitiveAnalyzer


def main():
    parser = argparse.ArgumentParser(
        description="Extract cannabis dispensary data from Dutchie platform (OPTIMIZED)",
...
Examples:
  ...
  %(prog)s dispensary-slug --output results/ --analysis
    ...
    print(f"🚀 Starting OPTIMIZED extraction for dispensary: {args.dispensary_slug}")
    ...
    try:
        ...
        print(f"✅ OPTIMIZED extraction completed: {len(results)} products extracted")
